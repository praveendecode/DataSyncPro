    ...
    def da_query(self):
        ...
        if option == "What are the Names of all the videos and their corresponding channels?":
            if st.button("Get Solution"):
                ...
                st.code(f"    Index       Channel Names                    Video Title")
                st.code(pd.DataFrame(data_1, columns=["", ""], index=range(1, len(data_1) + 1)))
                st.balloons()
                st.success("Solved", icon="✅")
                # st.info("INFO :\n In This Query We Have Seen All The Videos Title And Their Channel Names")
        # 2
            ...
            if st.button("Get Solution"):
                ...
                df_1 = pd.DataFrame(data_2, columns=["", ""], index=range(1, len(data_2) + 1))
                st.code(f"    Index      Channel Names  Total Videos")
                st.code(df_1)
                st.balloons()
                st.success("Solved", icon="✅")
                # name,count = df_1.loc[1, '']
                # st.info(f"INFO :\n In This Analysis We Have Seen The Channel {name} Has Posted {count} Videos as the most Count")
        ...
        # 3
            ...
            if st.button("Get Solution"):
                ...
                st.code(f"    Index      Channels                  Video Title")
                df_3 = pd.DataFrame(data_3, columns=['', ''], index=range(1, len(data_3) + 1))
                st.code(df_3)
                st.balloons()
                st.success("Solved", icon="✅")
                name, title = df_3.loc[1]
                # st.info(f"INFO:\nIn This Analysis We Have Seen The Video Title '{title}'.. In  '{name}'.. Channel  has got more views")
        # 4
        elif option == "How many comments were made on each video, and what are their corresponding video names?":
            if st.button("Get Solution"):
                query_4 = "select title ,comment_count from video  order by comment_count asc"
                cursor.execute(query_4)
                data_4 = [i for i in cursor.fetchall()]
                st.code(f"    Index                     Video Title                    Total Comments")
                st.code(pd.DataFrame(data_4, columns=["", ""], index=range(1, len(data_4) + 1)))
                st.balloons()
                st.success("Solved", icon="✅")
        # 5
            ...
            if st.button("Get Solution"):
                ...
                st.code(f"    Index      Channel Names                    Video Title")
                st.code(pd.DataFrame(data_5, columns=["", ""], index=range(1, len(data_5) + 1)))
                st.balloons()
                st.success("Solved", icon="✅")
        # 6
            ...
            if st.button("Get Solution"):
                ...
                st.code(f"    Index                           Title                      👍 👎")
                st.code(pd.DataFrame(data_6, columns=["", "", ""], index=range(1, len(data_6) + 1)))
                st.balloons()
                st.success("Solved", icon="✅")
        # 7
            ...
            if st.button("Get Solution"):
                ...
                st.code(f"    Index     Channel Names   Channel Views")
                st.code(pd.DataFrame(data_7, columns=["", ""], index=range(1, len(data_7) + 1)))
                st.balloons()
                st.success("Solved", icon="✅")
        # 8
        elif option == "What are the names of all the channels that have published videos in the year 2022?":
            if st.button("Get Solution"):
                query_8 = "select distinct(channel_name) , year_publishedat   from video where year_publishedat = 2022"
        ...
        # 9
            ...
            if st.button("Get Solution"):
                ...
                st.code(f"    Index      Channel Names   Average Video Duration In Seconds")
                st.code(pd.DataFrame(data_9, columns=["", ""], index=range(1, len(data_9) + 1)))
                st.balloons()
                st.success("Solved", icon="✅")
        # 10
        elif option == "Which videos have the highest number of comments, and what are their corresponding channel names?":
            if st.button("Get Solution"):
                query_10 = "select channel_name  , title from video order by comment_count desc limit 30"
                cursor.execute(query_10)
                data_10 = [i for i in cursor.fetchall()]
                st.code(f"    Index       Channel Names                 Video Title")
                st.code(pd.DataFrame(data_10, columns=["", ""], index=range(1, len(data_10) + 1)))
                st.balloons()
                st.success("Solved", icon="✅")
...
# title_html = """
#     <h1 style="color: blue;">My HTML Title</h1>
# """
#
